...
class CategoricalTransformer( BaseEstimator, TransformerMixin ):
  ...
  def transform(self, X , y = None ):
    ...
    if self.new_features: 
      ...
      df.loc[(df['neighbourhood_cleansed']!='Copacabana') &
             (df['neighbourhood_cleansed']!='Ipanema') &
             (df['neighbourhood_cleansed']!='Barra da Tijuca') &
             (df['neighbourhood_cleansed']!='Leblon') &
             (df['neighbourhood_cleansed']!='Botafogo') &
             (df['neighbourhood_cleansed']!='Jacarepaguá') &
             (df['neighbourhood_cleansed']!='Recreio dos Bandeirantes') &
             (df['neighbourhood_cleansed']!='Santa Teresa') &
             (df['neighbourhood_cleansed']!='Centro') &
             (df['neighbourhood_cleansed']!='Flamengo') ,'neighbourhood_cleansed'] = 'Other'
        
      df.loc[(df['property_type']!='Entire rental unit') &
             (df['property_type']!='Private room in rental unit') &
             (df['property_type']!='Entire condominium (condo)') &
             (df['property_type']!='Entire residential home') &
             (df['property_type']!='Private room in residential home') ,'property_type'] = 'Other' 
...
def process_args(args):
    ...
    x2 = x_train.select_dtypes("float64").copy()
    outlier_floats = lof.fit_predict(x2)
    mask = outlier_floats != -1
    ...
    fig, ax = plt.subplots()
    ax.scatter(predict, y_val, edgecolors=(0, 0, 1))
    ax.set_ylim([0, 200000])
    ax.plot([y_val.min(), 200000], [y_val.min(), 200000], 'r--', lw=3)
